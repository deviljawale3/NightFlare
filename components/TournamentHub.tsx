...
const TournamentHub: React.FC<TournamentHubProps> = ({ onBack }) => {
    ...
    return (
        <div className="fixed inset-0 z-[150] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 animate-in fade-in zoom-in duration-300">
            <div className="w-full max-w-6xl bg-[#0a0a0a] border border-white/10 rounded-[2rem] overflow-hidden flex flex-col shadow-2xl relative max-h-[90vh]">
                ...
                {/* Tabs */}
                <div className="relative z-10 flex border-b border-white/10 bg-black/40">
                    ...
                    {filteredTournaments.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {filteredTournaments.map((tournament) => (
                                <div
                                    key={tournament.id}
                                    className="bg-gradient-to-br from-yellow-900/10 to-orange-900/10 rounded-xl p-6 border border-yellow-500/20 hover:border-yellow-500/40 transition-all group"
                                >
                                    {/* Tournament Header */}
                                    ...
                                    {tournament.status === 'ACTIVE' && (
                                        <button
                                            ...
                                            className="w-full py-3 bg-gradient-to-r from-yellow-600 to-orange-600 rounded-lg text-white font-black text-sm uppercase tracking-wider hover:scale-[1.02] active:scale-95 transition-all shadow-lg disabled:opacity-50 disabled:grayscale disabled:cursor-not-allowed"
                                        >
                                    ...
                                    {tournament.status === 'UPCOMING' && (
                                        <div className="w-full py-3 bg-blue-600/20 rounded-lg text-blue-400 font-black text-sm uppercase tracking-wider text-center border border-blue-500/30">
                                            Starts Soon
                                        </div>
                                    )}

                                    {tournament.status === 'COMPLETED' && (
                                        <button className="w-full py-3 bg-white/5 rounded-lg text-white/50 font-black text-sm uppercase tracking-wider hover:bg-white/10 transition-all">
                                            View Results
                                        </button>
                                    )}
                                </div>
                ...
                {/* Footer */}
                <div className="p-6 border-t border-white/10 flex justify-between items-center relative z-10 bg-black/40">
