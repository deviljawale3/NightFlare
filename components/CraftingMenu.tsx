...
const CraftingMenu: React.FC<CraftingMenuProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/95 backdrop-blur-md z-[100] pointer-events-auto p-2 sm:p-4 md:p-8" onClick={onClose}>
      <div className="bg-[#050505] border-2 border-white/5 p-6 md:p-14 rounded-[2rem] md:rounded-[3.5rem] max-w-5xl w-full h-[90vh] md:h-auto overflow-hidden shadow-[0_0_200px_rgba(0,0,0,1)] animate-in fade-in zoom-in duration-300 relative flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-between items-start mb-6 md:mb-10">
          <div className="flex flex-col">
            ...
            <div className="flex gap-3 md:gap-6 mt-4 md:mt-6">
                <button onClick={() => setTab('FORGE')} className={`px-4 md:px-8 py-2 md:py-3 rounded-xl font-black text-[10px] md:text-xs uppercase tracking-widest transition-all ${tab === 'FORGE' ? 'bg-orange-600 text-white' : 'bg-white/5 text-white/40 border border-white/5'}`}>Forging</button>
                <button onClick={() => setTab('SKILLS')} className={`px-4 md:px-8 py-2 md:py-3 rounded-xl font-black text-[10px] md:text-xs uppercase tracking-widest transition-all ${tab === 'SKILLS' ? 'bg-orange-600 text-white' : 'bg-white/5 text-white/40 border border-white/5'}`}>Upgrades</button>
            </div>
          </div>
