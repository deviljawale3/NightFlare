...
const LevelClearMenu: React.FC = () => {
  const { score, level, nextLevel, kills, getNightName } = useGameStore();

  const getNextLevelName = () => {
    return getNightName(level + 1);
  };

  const currentLevelName = getNightName(level);
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center bg-[#050505] z-[150] pointer-events-auto overflow-hidden animate-in fade-in duration-700 font-['Outfit']">
      ...
      <div className="absolute inset-0 z-0 bg-[radial-gradient(ellipse_at_center,_rgba(10,5,25,1)_0%,_rgba(0,0,0,1)_100%)]" />
      ...
      <div className="absolute inset-0 z-0">
        <Canvas camera={{ position: [0, 1, 8], fov: 45 }}>
          ...
          <pointLight position={[-5, -10, -5]} intensity={2} color="#00f2ff" />

          <WarpTunnel />

          <Float speed={2.5} rotationIntensity={1.2} floatIntensity={0.8}>
            <group position={[0, -0.5, 4]} scale={0.75}>
              <Trophy />
            </group>
          </Float>
          ...
          <Sparkles count={100} scale={15} size={4} speed={0.6} opacity={0.5} color="#00f2ff" />
        </Canvas>
      </div>
      ...
      <div className="relative z-10 w-full max-w-2xl px-6 flex flex-col items-center text-center h-full justify-center pointer-events-none">
        ...
        <div className="mb-4 sm:mb-6 animate-in slide-in-from-top-12 duration-700">
          <h1 className="text-[12vw] sm:text-8xl font-black text-white italic tracking-tighter leading-none drop-shadow-[0_0_30px_rgba(0,242,255,0.6)]">
            VICTORY
          </h1>
          <div className="flex flex-col items-center mt-2">
            <div className="flex items-center gap-4">
              <div className="h-[2px] w-12 bg-gradient-to-r from-transparent to-cyan-400" />
              <p className="text-cyan-400 font-black tracking-[0.4em] text-[10px] sm:text-xs uppercase">
                {currentLevelName} SECURED
              </p>
        ...
        {/* 3D Progression Bar Sim (Visual only) */}
        <div className="w-full max-w-xs h-1 relative overflow-hidden bg-white/10 rounded-full mb-12 sm:mb-16 animate-in fade-in duration-1000 delay-500">
          <div className="absolute top-0 left-0 h-full bg-cyan-400 w-full animate-progress-reveal" />
          <style>{`
        ...
        <div className="grid grid-cols-2 gap-4 w-full max-w-md mb-12 animate-in slide-in-from-bottom-8 duration-700 delay-300">
          <div className="bg-black/60 backdrop-blur-2xl p-6 rounded-3xl border border-white/10 shadow-2xl group hover:bg-white/5 transition-all">
            <div className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1 text-left">EXP EARNED</div>
            <div className="text-white text-3xl sm:text-4xl font-black text-left tabular-nums tracking-tighter group-hover:text-cyan-400 transition-colors">+{score.toLocaleString()}</div>
          </div>
          <div className="bg-black/60 backdrop-blur-2xl p-6 rounded-3xl border border-white/10 shadow-2xl group hover:bg-white/5 transition-all">
            <div className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1 text-left">ENEMIES PURGED</div>
        ...
        <div className="w-full max-w-sm animate-in slide-in-from-bottom-12 duration-700 delay-500">
          <div className="text-white/60 text-[10px] sm:text-xs font-black uppercase tracking-[0.3em] mb-5 flex items-center justify-center gap-3">
            <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_10px_#22c55e]" />
            NEXT SECTOR: <span className="text-white">{getNextLevelName()}</span>
          </div>

          <button
            onClick={nextLevel}
            className="w-full group relative bg-cyan-400 text-black py-4 sm:py-6 rounded-[2rem] font-black text-lg sm:text-2xl hover:scale-[1.05] active:scale-95 transition-all shadow-[0_10px_50px_rgba(0,242,255,0.4)] overflow-hidden italic tracking-tighter pointer-events-auto"
          >
            <span className="relative z-10 flex items-center justify-center gap-3">
              ASCEND TO NEXT LEVEL <span className="text-xl opacity-60 group-hover:translate-x-2 transition-transform">➔</span>
            </span>
        ...
        <div className="absolute bottom-8 opacity-40 hover:opacity-100 transition-opacity">
          <DeeJayLabsLogo className="scale-125" />
        </div>
      </div>
    </div>
  );
